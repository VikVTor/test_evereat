DROP DATABASE IF EXISTS kebab_project;
CREATE DATABASE kebab_project;
...
CREATE TABLE admin (
    id_admin VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    username VARCHAR(64),
    password VARCHAR(256),
    email VARCHAR(128),
    sales BOOLEAN DEFAULT (1),
    edit BOOLEAN DEFAULT (1),
    creates BOOLEAN DEFAULT (1),
    archive BOOLEAN DEFAULT (1),
    stats BOOLEAN DEFAULT (1),
    user_details BOOLEAN DEFAULT (1)
);

CREATE TABLE restaurant (
    id_restaurant VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
...
CREATE TABLE user (
    id_user VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    name VARCHAR(64),
    surname VARCHAR(64),
    email VARCHAR(128),
    telephone VARCHAR(64),
...
CREATE TABLE rider (
    id_rider VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    name VARCHAR(64),
    surname VARCHAR(64),
    email VARCHAR(128),
    password VARCHAR(256),
...
CREATE TABLE reviews (
    id_review VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    stars INT,
    date DATE,
    text VARCHAR(2048),
    id_user VARCHAR(64),
...
CREATE TABLE deliver_options (
    id_deliver_opt VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    delivery_price INT,
...
CREATE TABLE business_hours (
    id_business_hours VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    day VARCHAR(32),
    open_hour TIME,
    close_hour TIME,
    id_restaurant VARCHAR(64),
...
CREATE TABLE menu (
    id_menu VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    id_restaurant VARCHAR(64),
    FOREIGN KEY (id_restaurant) REFERENCES restaurant (id_restaurant)
);

CREATE TABLE dish (
    id_dish VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    name VARCHAR(64),
    id_menu VARCHAR(64),
...
CREATE TABLE variant (
    id_variant VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    name VARCHAR(64),
    img VARCHAR(64),
...
CREATE TABLE extra (
    id_extra VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    name VARCHAR(64),
    id_dish VARCHAR(64),
    FOREIGN KEY (id_dish) REFERENCES dish (id_dish)
);
...
CREATE TABLE order_state (
    id_state VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    state VARCHAR(256)
);

CREATE TABLE payment_method (
    id_payment_method VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    payment_method VARCHAR(256)
);

CREATE TABLE `order` (
    id_order VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    date DATE,
    hour TIME,
    paid BOOLEAN,
    id_user VARCHAR(64),
...
CREATE TABLE restaurant_photo (
    id_foto VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
...
CREATE TABLE discount (
    id_discount VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
    discount INT, -- entity of the discount in percentage (eg. 17% -> 17)
    start_date DATE DEFAULT "2023-11-20",
    end_date DATE DEFAULT "2023-11-30",
    with_code BOOLEAN DEFAULT 0, -- boolean to check if the discount can be used only with a code or no
...
CREATE TABLE gift (
    id_gift VARCHAR(64) DEFAULT (uuid()) PRIMARY KEY,
